...
export const ConfigSchema = z.object({
  ...
  AETHERDUST_METRICS_ENABLED: bool.default(true).describe('expose Prometheus /metrics on the api and the worker’s private port'),
  AETHERDUST_METRICS_TOKEN: z.string().min(8).optional().describe('bearer token for /metrics, as an alternative to the admin token (which is always accepted)'),
  AETHERDUST_METRICS_PUBLIC: bool.default(false).describe('serve /metrics without any token. Only on a private network: the exposition names applications, budgets and the sponsor balance'),
  AETHERDUST_DASHBOARD_ORIGIN: z.string().optional().describe('comma-separated origins allowed to call the api from a browser; unset = any (MVP default)'),
